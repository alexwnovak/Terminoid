#==============================================================================
# Disable-TerminoidPrompt Compatibility Tests
...
Describe 'Disable-TerminoidPrompt compatibility' {
    BeforeEach {
        Get-Module -Name Terminoid -All | Remove-Module -Force -ErrorAction Ignore
        Import-Module $PSScriptRoot\..\..\Terminoid.psd1 -Force
    }
    It 'exposes a function named Disable-TerminoidPrompt' {
        Get-Command Disable-TerminoidPrompt -Module Terminoid | Should -Not -Be $null
    }
    It 'exposes exactly 0 parameters' {
        (Get-Command Disable-TerminoidPrompt).Parameters.Count | Should -Be 0
    }
}
